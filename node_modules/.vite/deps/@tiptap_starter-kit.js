  ...
  addKeyboardShortcuts() {
    return {
      ...
      "Shift-Tab": ({ editor }) => {
        ...
        const { $from, empty } = selection;
        if ($from.parent.type !== this.type) return false;
        if (empty) return editor.commands.command(({ tr: tr2 }) => {
      ...
      ArrowDown: ({ editor }) => {
        ...
        if (doc.nodeAt(after)) return editor.commands.command(({ tr: tr2 }) => {
...
var CharacterCount = Extension.create({
  ...
  onBeforeCreate() {
    this.storage.characters = (options2) => {
      ...
      if (((options2 === null || options2 === void 0 ? void 0 : options2.mode) || this.options.mode) === "textSize") {
...
function createPlaceholderDecoration(options2) {
  const { editor, placeholder, dataAttribute, pos, node, isEmptyDoc, hasAnchor, classes: { emptyNode, emptyEditor } } = options2;
...
function scanRangeForDecorations({ editor, options: options2, dataAttribute, doc, selection, from: from2, to }) {
  const { anchor } = selection;
  const decorations = [];
  ...
  doc.nodesBetween(from2, to, (node, pos) => {
    ...
    if (!node.type.isTextblock) return options2.includeChildren;
    if ((hasAnchor || !options2.showOnlyCurrent) && isEmpty) decorations.push(createPlaceholderDecoration({
      editor,
      isEmptyDoc,
      dataAttribute,
      hasAnchor,
...
function buildPlaceholderDecorations({ editor, options: options2, dataAttribute, doc, selection }) {
  if (!(editor.isEditable || !options2.showOnlyWhenEditable)) return null;
  const { anchor } = selection;
  const decorations = [];
  const isEmptyDoc = editor.isEmpty;
  if (options2.showOnlyCurrent && !options2.includeChildren) {
...
function updateDecorationsInRanges({ decorations, ranges, editor, options: options2, dataAttribute, doc, selection }) {
...
function createPlaceholderStateField({ editor, options: options2, dataAttribute }) {
  return {
    init(_config, state) {
...
function createPlaceholderPlugin({ editor, options: options2 }) {
  ...
  return new Plugin({
    ...
    props: { decorations: useResolvedPath ? ({ doc, selection }) => buildPlaceholderDecorations({
      ...
      if (options2.showOnlyWhenEditable && !editor.isEditable) return DecorationSet.empty;
    ...
    return [new Plugin({
      ...
      props: {
        decorations(state) {
          ...
          return DecorationSet.create(state.doc, [Decoration.inline(state.selection.from, state.selection.to, { class: options2.className })]);
        },
        handleDOMEvents: {
          blur(view) {
